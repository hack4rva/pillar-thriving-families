...
- Contain only grounded claims (no invented data about Richmond programs or services)
- Cite sources where possible (inline citation numbers from research tools are acceptable; include URLs if available)
...
```markdown
- [`SECTION_N_topic_slug.md`](SECTION_N_topic_slug.md) — Short description matching the report title
```
...
Add to the Confirmed section:
```
- [claim description] | [official URL] | accessed [date]
```

If the report corrects a previously noted claim, update the existing entry with a correction note rather than deleting it.
