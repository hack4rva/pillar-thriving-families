...
* **Maternal Data is Robust:** VDH updated its Maternal & Child Health dashboards on October 10, 2025 [5]. The data is highly granular, showing 5.8 infant deaths per 1,000 live births, 34.5 maternal deaths per 100,000 live births (2019-2023), and revealing that 40% of pregnancy-associated deaths (2018-2022) were due to accidental overdose [5].
...
Teams should resolve these unknowns within the first three hours of the hackathon:
* **Unknown:** Are there specific API or iFrame constraints for embedding VDH dashboard elements directly? (Fast-track: Ping a VDH data contact or default to static snapshots with deep links).
* **Unknown:** Does the Office of Community Wealth Building (OCWB) have a JSON/CSV feed of youth programs, or must it be scraped/manually curated from the YES website? [7] (Fast-track: Ask City mentors immediately; if no, manually curate top 10 programs).
