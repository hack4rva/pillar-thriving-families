...
### Unknowns
* **API Stability**: It is unknown if the Census ACS API will experience rate limiting or downtime during the specific 48-hour hackathon window, necessitating the CSV-fallback approach.
* **Granular Maternal Data Availability**: It is unknown if VDH will approve requests for tract-level maternal outcome data (rather than locality-level) for public dashboarding, given their strict confidentiality and suppression policies [13].
* **PRAMS Micro-Targeting**: While PRAMS oversamples Richmond [7], it is unknown if the sample size is large enough to allow for cross-tabulation by both race *and* specific Richmond zip codes without triggering suppression rules.
...
8. *American Community Survey 5-Year Data (2009-2024)*. https://www.census.gov/data/developers/data-sets/acs-5year.html
9. *DP05ACS Demographic and Housing Estimates - Census Data*. https://data.census.gov/table/ACSDP5Y2024.DP05?g=060XX00US5115991571
